...
void corp(string filename)
{
	...
	for (int i = 0; i < img.rows; ++i)
		for (int j = 0; j < img.cols; ++j){
		double d = d = (i - center[0]) * (i - center[0]) + (j - center[1]) * (j - center[1]);
		...
		{
			if      ((i < upperleft  [0] || j < upperleft  [1]) && d > upperleft  [2]) upperleft[0] = i, upperleft[1] = j     ,upperleft[2] =    d;
			else if ((i < upperright[0] || j > upperright[1])   && d > upperright [2]) upperright[0] = i, upperright[1] = j   ,upperright[2] =   d;
			else if ((i > bottonleft[0] || j < bottonleft[1])   && d > bottonleft [2]) bottonleft[0] = i, bottonleft[1] = j   ,bottonleft[2] =   d;
			else if ((i > bottonright[0] && j > bottonright[1]) && d > bottonright[2]) bottonright[0] = i, bottonright[1] = j ,bottonright[2] =  d;
		}
		}
